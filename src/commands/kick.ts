...
const command: Command = {
  ...
  async execute(interaction) {
    ...
    if (!member.kickable) {
      await interaction.reply({ content: 'I cannot kick that member. They may have higher permissions or roles.', flags: MessageFlags.Ephemeral });
      return;
    }
    ...
    await interaction.reply({ content: `Kicked ${user.tag}. Case #${caseEntry.id}.`, flags: MessageFlags.Ephemeral });
